...
def demo_v2_result(*, closed: bool = False, decision: str | None = None) -> BDV2AnalysisResult:
    ...
    stage_claim = "프로젝트 종료" if is_closed else "시공" if needs_confirm or is_monitor else "설계 - DD (기본)"
    ...
    gate = StageGate(
        status="closed" if is_closed else "local_action" if needs_confirm or is_monitor else "golden_time",
        research_depth="limited" if is_closed or needs_confirm or is_monitor else "deep",
        active_pursuit="stop" if is_closed else "limited" if needs_confirm or is_monitor else "active",
        historical_intelligence="limited" if is_closed else "none",
        rationale="Mock stage gate for v2 UI validation.",
        remaining_window="과거 구조 학습" if is_closed else "남은 범위 확인" if needs_confirm or is_monitor else "사양 영향 가능 시기",
        recommended_focus="과거 참여사 확인" if is_closed else "구매경로 확인" if needs_confirm or is_monitor else "설계·EPC 의사결정 구조",
    )
    stakeholders = [
        BDV2Stakeholder(
            role="EPC" if is_closed else "Owner Project Manager",
    ...
    return BDV2AnalysisResult(
        ...
        bd_decision=BusinessDevelopmentDecision(
            decision=mode,
            headline={"now":"NOW · PRIORITY WINDOW","needs_confirm":"NEEDS CONFIRM · Remaining Scope","monitor":"MONITOR · Wait for Trigger","closed":"CLOSED · Active Pursuit Stop"}[mode],
            rationale="Mock decision based on stage and open-scope evidence.",
            context_basis=["신축", stage_claim, "EPC 선정 상태"],
            evidence_labels=["Demo source"],
            action_direction={"now":"설계·Partner 구조에 즉시 개입","needs_confirm":"남은 범위를 확인","monitor":"Tender와 Actor 선정을 추적","closed":"다음 기회를 위한 구조 학습"}[mode],
        ...
        project_intelligence=ProjectIntelligence(
            ...
            open_scopes=[IntelligenceItem(claim="EPC package status requires confirmation",credibility="unknown",rationale="No current award source",evidence_labels=["Demo source"],temporal_scope="current")] if needs_confirm else [],
        ...
        relationship_map=RelationshipDecisionMap(
            actors=[
                ProjectActor(actor_id="owner",role="Owner / End Client",organization="Demo Owner",temporal_scope="current",participation_status="Current owner",credibility="confirmed",evidence_labels=["Demo source"]),
                ProjectActor(actor_id="epc",role="EPC",organization="Demo Engineering",temporal_scope="historical" if is_closed else "candidate",participation_status="Current participation unconfirmed",credibility="hypothesis",evidence_labels=["Demo historical source"]),
            ],
